...
app.post('/login', async (req, res) => {
    ...
    try {
        const [exRows] = await db.execute(
            'SELECT cliente_id, correo, password FROM Clientes_EX WHERE correo = ?',
            [email]
        );
        ...
        if (exRows.length > 0) {
            ...
            if (passMatch) {
                return res.json({ 
                    status: 'success', 
                    message: 'Login exitoso (EX)!', 
                    tipo: 'EX',
                    cliente_id: userEX.cliente_id 
                });
            } else {
        ...
        const [mxRows] = await db.execute(
            'SELECT cliente_id, correo, password FROM Clientes_MX WHERE correo = ?',
            [email]
        );

        if (mxRows.length > 0) {
            const userMX = mxRows[0];
            const passMatch = await bcrypt.compare(password, userMX.password);

            if (passMatch) {
                return res.json({ 
                    status: 'success', 
                    message: 'Login exitoso (MX)!', 
                    tipo: 'MX',
                    cliente_id: userMX.cliente_id 
                });
            } else {
                return res.status(401).json({ status: 'error', message: 'Contraseña incorrecta.' });
            }
